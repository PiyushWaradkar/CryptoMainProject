...
def TechnicalAnalysis(coinid):
    try:
        ...
        '''
        if datetime.today() != sync_date:
            global flag
            flag=0

        if datetime.today() == sync_date and flag == 0:
            for temp_coin_name in coinlist:
                func_api.run(temp_coin_name)
            global sync_date 
            global flag
            sync_date = sync_date + time_d
            flag=1
        '''
